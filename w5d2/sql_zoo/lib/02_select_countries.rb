...
def high_population_gdps
  ...
  execute(<<-SQL)
    SELECT countries.name, (countries.gdp / countries.population) AS per_capita_gdp
    FROM countries
    WHERE countries.population >= 200000000;
  SQL
end
...
def population_in_millions
  ...
  # millions.
  execute(<<-SQL)
    SELECT countries.name, (countries.population / 1000000) AS population_in_millions
    FROM countries
